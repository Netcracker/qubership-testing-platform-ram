...
public class V17AddCreatedDateIndexOnBrowserLogRecordCollection {

    /**
     * Add ttl index to logrecordBrowserConsoleLogs collection.
     */
    @ChangeSet(order = 1)
    public void addCreatedDateIndex(MongoTemplate mongoTemplate) {
        String processName = V17AddCreatedDateIndexOnBrowserLogRecordCollection.class.getName();
        ...
        Index myIndex = new Index()
                ...
                .expire(259200, TimeUnit.SECONDS)
                .background();

        DefaultIndexOperations indexOperations = new DefaultIndexOperations(
                mongoTemplate,
                MigrationConstants.LOG_RECORD_BROWSER_CONSOLE_LOGS_COLLECTION_NAME,
                null
        );
        try {
