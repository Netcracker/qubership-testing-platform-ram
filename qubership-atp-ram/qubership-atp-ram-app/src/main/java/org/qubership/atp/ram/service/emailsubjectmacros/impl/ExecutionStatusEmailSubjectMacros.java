...
@Slf4j
@Component
public class ExecutionStatusEmailSubjectMacros implements ResolvableEmailSubjectMacros {
    ...
    public String resolve(ExecutionRequest executionRequest, ExecutionSummaryResponse executionSummaryResponse) {
        ...
        log.debug("Start resolving macros 'EXECUTION_REQUEST_STATUS' for execution request with id: {}",
                executionRequestId);

        String executionStatus = executionRequest.getExecutionStatus().getName();
        log.debug("Resolved value: {}", executionStatus);
